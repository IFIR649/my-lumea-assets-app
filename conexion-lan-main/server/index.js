...
function getClientIdentity(req, opts = {}) {
  ...
  const idSource = allowQuery ? req.query?.clientId || req.headers["x-client-id"] : req.headers["x-client-id"];
  const nameSource = allowQuery ? req.query?.clientName || req.headers["x-client-name"] : req.headers["x-client-name"];
...
app.get("/api/admin/port-check/:port", adminOnly, (req, res) => {
  ...
  exec(`netstat -ano | findstr :${port}`, { windowsHide: true, maxBuffer: 256 * 1024 }, (_err, stdout) => {
...
app.get("/api/admin/chat/export.csv", adminOnly, (req, res) => {
  ...
  const body = rows
    .map((row) => [row.kind, row.from_id, row.from_name, row.to_id, row.text, row.created_at].map(esc).join(","))
    .join("\n");
...
app.get("/api/notes/search", (req, res) => {
  ...
  if (q) {
    where.push("(COALESCE(title, '') LIKE @q OR content LIKE @q OR COALESCE(client_name, '') LIKE @q)");
    params.q = `%${q}%`;
  }
...
app.get("/api/chat/stream", (req, res) => {
  ...
  sendSse(res, "hello", { ok: true, you: { clientId, clientName: getSafeName(clientId, clientName) } });

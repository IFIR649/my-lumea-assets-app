...
export function ChatPanel({ toast }: { toast: ToastFn }) {
    ...
    try {
      if (value.startsWith("@")) {
        ...
        if (targetUser) {
          ...
          setMentionFilter("");
          setEmojiOpen(false);
          toast("success", result.delivered ? "Susurro enviado." : "Susurro guardado (destino no conectado).");
          return;
        }
      }
  ...
  const filteredUsers = users.filter((u) => u.clientName.toLowerCase().includes(mentionFilter)).slice(0, 5);
    ...
    <Card>
      ...
      <CardBody>
        <div className="mb-3 rounded-2xl border border-amber-200 bg-amber-50 p-3 text-xs text-amber-900">
          Aviso: mensajes auditables por el administrador local. Si no es DM, cualquiera puede verlo.
        </div>
        ...
        <div className="h-[360px] space-y-2 overflow-auto rounded-2xl border border-slate-200 bg-white/60 p-3">
          {msgs.map((m) => {
            ...
            return (
              <div key={`${m.id}-${m.ts}`} className={mine ? "flex justify-end" : "flex justify-start"}>
                <div
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-[11px] text-slate-500">{new Date(m.ts).toLocaleTimeString()}</div>
                    </div>
                  </div>
            ...
            <div className="absolute bottom-full left-0 z-10 mb-2 w-64 rounded-xl border border-slate-200 bg-white p-1 shadow-lg">
              ...
              {filteredUsers.map((u) => (
                <button
                  ...
                  type="button"
                >
                  <div className={`h-2 w-2 rounded-full ${u.online ? "bg-emerald-400" : "bg-slate-300"}`} />

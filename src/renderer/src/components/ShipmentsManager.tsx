...
type ShipmentDetailTab = 'summary' | 'quote' | 'guides' | 'activity'
type ShipmentsMainView = 'boxes' | 'pending' | 'approved'
...
function ShipmentSummaryPanel({ children }: { children: React.ReactNode }): React.JSX.Element {
  return <div className="min-w-0 space-y-4">{children}</div>
}

function ShipmentQuotePanel({ children }: { children: React.ReactNode }): React.JSX.Element {
  return <div className="min-w-0 space-y-4">{children}</div>
}

function ShipmentGuidesPanel({ children }: { children: React.ReactNode }): React.JSX.Element {
  return <div className="min-w-0 space-y-4">{children}</div>
}

function ShipmentActivityPanel({ children }: { children: React.ReactNode }): React.JSX.Element {
  return <div className="min-w-0 space-y-4">{children}</div>
}
...
export default function ShipmentsManager(): React.JSX.Element {
  ...
  const [mainView, setMainView] = useState<ShipmentsMainView>('boxes')
  ...
  const [detailTab, setDetailTab] = useState<ShipmentDetailTab>('summary')
  ...
  const detailTabs: Array<{ value: ShipmentDetailTab; label: string }> = [
    { value: 'summary', label: 'Resumen' },
    { value: 'quote', label: 'Cotizar/Aprobar' },
    { value: 'guides', label: 'Guias/PDF' },
    { value: 'activity', label: 'Items/Eventos' }
  ]
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-6">
      ...
      <section className="rounded-3xl border border-white/5 bg-white/[0.02] p-3 sm:p-4">
        <div className="flex flex-wrap gap-2">
          <button
            type="button"
            onClick={() => setMainView('boxes')}
            className={cn(
          ...
          >
            Envios realizados ({approved.length})
          </button>
        </div>
      </section>
      ...
      {isDetailModalOpen && (
        <div className="fixed inset-0 z-40 flex items-stretch justify-center bg-black/70 p-0 sm:p-4">
          <div className="flex h-full w-full flex-col border border-white/10 bg-surface100 shadow-2xl sm:h-[92vh] sm:max-w-6xl sm:rounded-3xl">
            <div className="flex items-center justify-between gap-3 border-b border-white/10 px-4 py-3 sm:px-5">
              ...
              <button
                ...
                className="rounded-2xl bg-white/5 px-4 py-2 text-sm font-semibold text-zinc-200 hover:bg-white/10"
              >
                Cerrar
              </button>
            </div>
